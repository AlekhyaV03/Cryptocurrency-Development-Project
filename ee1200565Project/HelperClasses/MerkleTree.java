...
public class MerkleTree {
    ...
    //extra method
    public List<Pair<String,String>> getSiblingCoupledPathToRoot(int doc_idx) {
        List<Pair<String,String>> path=new ArrayList<Pair<String,String>>();
        TreeNode temp = rootnode;
        if(temp == null){
            return path;
        }
        int levelVal = numdocs/2;
        int num = numdocs/2;
        path.add(new Pair<String,String>(temp.val,null));
        while(levelVal != 0){
            levelVal = levelVal/2;
            path.add(new Pair<String,String>(temp.left.val,temp.right.val));
            if(doc_idx <= num) {
                temp = temp.left;
                num = num-levelVal;
            }
